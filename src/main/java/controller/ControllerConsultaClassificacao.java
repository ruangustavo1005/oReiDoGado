...
import view.ViewConsultaClassificacao;

/**
 * @author Ruan
 */
public class ControllerConsultaClassificacao extends ControllerConsulta<Classificacao, ViewConsultaClassificacao, Dao> {

    public ControllerConsultaClassificacao(Controller caller) {
        super(caller);
    }
    ...
    @Override
    public ViewConsultaClassificacao getInstanceView() {
        return new ViewConsultaClassificacao();
    }
    ...
    private void addListenerAcaoInserir() {
        this.getView().getBotaoInserir().addActionListener((e) -> {
            (new ControllerManutencaoClassificacao(this, false)).montaTela();
        });
    }
    
    private void addListenerAcaoAlterar() {
        this.getView().getBotaoAlterar().addActionListener((e) -> {
            (new ControllerManutencaoClassificacao(this, false)).setModel(this.getSelectedModel()).montaTela();
        });
    }
    ...
    private void addListenerAcaoExcluir() {
        this.getView().getBotaoExcluir().addActionListener((e) -> {
            if (this.getView().showDialog("Deseja excluir os registros selecionados?", "Atenção", JOptionPane.OK_CANCEL_OPTION) == JOptionPane.OK_OPTION) {
                ...
                for (Classificacao selectedModel : this.getSelectedModels()) {
    ...
    private void addListenerAcaoVisualizar() {
        this.getView().getBotaoVisualizar().addActionListener((e) -> {
            (new ControllerManutencaoClassificacao(this, true)).setModel(this.getSelectedModel()).montaTela();
        });
    }
